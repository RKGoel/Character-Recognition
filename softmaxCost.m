function [J grad] = softmaxCost(nn_params, ...
                                   input_layer_size, ...
                                   hidden_layer_size, ...
                                   num_labels, ...
                                   X, y, lambda)
...
z1=Theta1*a1';
a2=softmax(z1); % value for hidden layer

a2=a2';
a2=[ones(m,1),a2];
z2=Theta2*a2';
a3=softmax(z2); % value for output layer
...
tmp = full(sparse(y, 1:m, 1));
const=(lambda*sum((sum(Theta2.^2)-Theta2(1).^2)))/2;

cost = - mean(sum(tmp .* log(hx))) + (lambda/2.0) * sum(sumsq(Theta2));

cost=-cost/m + const;

J = cost;
%Thetagrad=((hx-tmp)*data')/m+lambda*theta;
